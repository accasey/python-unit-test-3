...
from SingleSignOn.my_service import MyService
from SingleSignOn.request import Request
from SingleSignOn.ssotoken import SSOToken
from SingleSignOn.single_sign_on import SingleSignOnRegistry


def test_hello_name():
    stub_sso_registry = Mock(SingleSignOnRegistry)
    service = MyService(stub_sso_registry)
    response = service.handle(Request("Andrew"), SSOToken())
    assert response.text == "Hello Andrew"
...
def test_single_sign_on():
    ...
    token = SSOToken()
    service.handle(Request("Andrew"), token)
...
def test_single_sign_on_with_invalid_token():
    ...
    service = MyService(spy_sso_registry)
    token = SSOToken()
    response = service.handle(Request("Andrew"), token)
    spy_sso_registry.is_valid.assert_called_with(token)
...
def test_single_sign_in_receives_correct_token():
    ...
    service.handle(Request("Andrew"), correct_token)
